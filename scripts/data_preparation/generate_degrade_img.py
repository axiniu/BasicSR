...
#dataset_name = 'Set5'
root_path = '/workspace/hdd/datasets'
dataset_names = ['Urban100', 'Set5', 'Set14', 'BSD'] # div2k set5 set14 B100 Urban100 可以处理的数据集
...
for dataset_name in dataset_names:
    ...
    if dataset_name == 'div2k':
        ...
        path_root_set5 = os.path.join(root_path, 'super_resolution/Set5/')#
        hr_names = glob.glob(os.path.join(path_root_set5, 'HR', '*.png'))
        hr_names.sort()
        save_path_root = os.path.join(path_root_set5, 'HR_with_degrade')
    elif dataset_name == 'Set14':
        path_root_set14 = os.path.join(root_path, 'super_resolution/Set14/')
        hr_names = glob.glob(os.path.join(path_root_set14,'HR','*.png'))
        hr_names.sort()
        save_path_root = os.path.join(path_root_set14, 'HR_with_degrade')
    elif dataset_name == 'BSD':
        path_root_b100 = os.path.join(root_path, 'super_resolution/BSD/')#/mnt/hdd/zhangkang/datasets/super_resolution/BSD
        hr_names = glob.glob(os.path.join(path_root_b100, 'HR', '*.png'))
        hr_names.sort()
        save_path_root = os.path.join(path_root_b100, 'HR_with_degrade')
    elif dataset_name == 'Urban100':
        path_root_urban100 = os.path.join(root_path, 'super_resolution/urban100/')
        hr_names = glob.glob(os.path.join(path_root_urban100, 'HR', '*.png'))
        hr_names.sort()
        save_path_root = os.path.join(path_root_urban100, 'HR_with_degrade')
    else:
        raise TypeError
